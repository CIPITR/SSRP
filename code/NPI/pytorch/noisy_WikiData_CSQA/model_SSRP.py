...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  9 10:26:13 2018
...
class NPI(nn.Module):
    def __init__(self, params, none_argtype_index, num_argtypes, num_programs,
                 ...
                 program_to_argtype_table, program_to_targettype_table,USE_DEVICE):
...
#            input_1 = self.target_var_key_and_embedding_arg_layer[current_argtype_id](temp)
    ...
    def train(self, feed_dict2):
        with torch.no_grad():
            ...
            mask_cnf = torch.tensor(feed_dict2['mask_reinforce'], dtype=self.dtype_float, device=self.device)
...
#            loss_reinforce = torch.sum(torch.sum(loss_reinforce, dim = 0))/(1+torch.sum(mask_cnf))
            loss_reinforce = torch.sum(torch.mean(loss_reinforce, dim = 0))
            ##########

            #coming back to intermediate reward part
            #mask_ir = self.IfPosIntermediateReward*mask_cnf
            #scaling_term_2 = torch.mul(mask_ir,torch.mul((int_reward - final_baseline) ,unbackpropable_intermediate_prob)).detach()
            ...
            #loss_ir = torch.sum(torch.sum(loss_ir, dim = 0))/(1+torch.sum(mask_ir))
            ...
            #loss_relaxed_reinforce = torch.mul(loss_relaxed_reinforce,mask_cnf)
            ...
            #loss_relaxed_reinforce = torch.sum(torch.sum(loss_relaxed_reinforce,dim=0))/(1+torch.sum(mask_ir))
