    ...
    def perform_training(self, batch_dict, epoch, overall_step_count):
        ...
        def dummy(input_model,feed_dict1):
            ...
            return reward, program_probabilities, logprogram_probabilities, per_step_probs, entropy, \
                    ...
                    intermediate_rewards, relaxed_rewards, program_type
        ...
        [reward, program_probabilities, logprogram_probabilities, \
         ...
         intermediate_rewards, relaxed_rewards,Cur_P_type] = dummy(self.model,feed_dict1)
        self.blockPrint()
        ...
        for beam_id in range(self.param['beam_size']):
            final_1.append(torch.stack(Cur_P_type[beam_id],dim=0))
        ...
        mask1 = torch.where(temp>0,temp,torch.zeros_like(r))
        mask2 = torch.where(r>0,torch.ones_like(r),torch.zeros_like(r))
        mask = mask2 + (1-mask2)*mask1
        #mask = mask1
...
#        mask = mask/torch.sum(mask,dim=1,keepdim=True)
        print 100*"--"
        temp = mask.data.cpu().numpy()
        print temp[0]
