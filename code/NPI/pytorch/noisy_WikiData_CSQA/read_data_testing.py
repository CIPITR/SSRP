...
#       self.noop_argtype_index = 0
        ...
        self.question_type = param['question_type']
...
#               self.program_type_vocab[parts[0]] = len(self.program_type_vocab)
    ...
    def get_batch_data(self, data_dict):
        ...
        try:
            all_ents_predicted = [data_dict[i][16] for i in range(num_data)]
            all_rels_predicted = [data_dict[i][17] for i in range(num_data)]
            all_types_predicted = [data_dict[i][18] for i in range(num_data)]
            all_predicted  = [True if ((self.question_type=="simple,logical" or self.question_type=="simple" or self.question_type=="logical") and all_ents_predicted[i] and all_rels_predicted[i] and all_types_predicted[i]) or self.question_type=="verify" or (self.question_type=="quantitative" and all_rels_predicted[i] and all_types_predicted[i]) else False for i in range(num_data)]
        except:
            all_predicted = [True for i in range(num_data)]
        batch_response_type = []
        ...
        return all_predicted, batch_ids, batch_orig_context, batch_context_nonkb_words, batch_context_kb_words, \
