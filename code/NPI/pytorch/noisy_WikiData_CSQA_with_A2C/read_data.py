...
#       self.noop_argtype_index = 0
        ...
        if not param['use_question_template']:
            self.use_question_template = False
        else:
            self.use_question_template = True
...
#       self.program_type_vocab = {self.none_progtype:self.none_progtype_index, self.noop_progtype:self.noop_progtype_index}
        ...
        try:
            self.template_vocab = pkl.load(open(param['template_vocab_file'],'rb'))
        except:
            self.template_vocab = pkl.load(open(param['vocab_file'],'rb'))
        self.template_vocab_inv = {v:k for k,v in self.template_vocab.items()}
        self.vocab_size = len(self.vocab)
        self.vocab_init_embed = np.empty([len(self.vocab), param['text_embed_dim']], dtype=np.float32)
        ...
        self.template_init_embed = np.empty([len(self.template_vocab), param['text_embed_dim']]).astype(np.float32)
        for i in xrange(self.template_init_embed.shape[0]):
            if self.template_vocab[i] in self.word2vec_pretrain_embed:
                self.template_init_embed[i,:] = self.word2vec_pretrain_embed[self.template_vocab[i]]
            else:
                self.template_init_embed[i,:] = np.random.rand(1, self.template_init_embed.shape[1]).astype(np.float32)
        count = 1
        self.ent_index = None
        self.rel_index = None
...
#               self.program_type_vocab[parts[0]] = len(self.program_type_vocab)
    ...
    def pad_or_clip(self, arr):
        arr_new = []
        for arri in arr:
            arri = [self.template_vocab_inv[self.start]]+arri
            if len(arri)>self.maxlen:
                arri = arri[:self.maxlen-1]+[self.template_vocab_inv[self.end]]
            elif len(arri)<self.maxlen:
                arri = arri + [self.template_vocab_inv[self.end]] + [self.template_vocab_inv[self.pad]]*(self.maxlen-1-len(arri))
            arr_new.append(arri)
        return np.asarray(arr_new, dtype=np.int32)
    ...
    def get_batch_data(self, data_dict):
        ...
        batch_context_template = [data_dict[i][14] for i in range(num_data)]
        batch_ids = [data_dict[i][15] for i in range(num_data)]
        batch_context_template = [[self.template_vocab_inv[w] if w in self.template_vocab_inv else self.template_vocab_inv[self.unk] for w in template_i.strip().split(' ')] for template_i in batch_context_template]    
        batch_context_template_str = [' '.join([str(x) for x in template_i]).strip() for template_i in batch_context_template]
        batch_context_template = self.pad_or_clip(batch_context_template)
        batch_response_type = []
        ...
        batch_context_ints = np.asarray([[i if i==self.pad_kb_symbol_index else text2int(i) for i in context_int[:self.max_num_var]] for context_int in batch_context_ints])
        ...
        batch_context_template = np.transpose(batch_context_template, (1,0))
        if self.use_question_template:
            batch_context_nonkb_words = batch_context_template
        ...
        return batch_ids, batch_context_template_str, batch_orig_context, batch_context_template, batch_context_nonkb_words, batch_context_kb_words, \
