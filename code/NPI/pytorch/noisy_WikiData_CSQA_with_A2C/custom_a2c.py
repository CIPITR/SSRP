...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  2 18:45:22 2018
...
class NPI(nn.Module):
    def __init__(self, params, none_argtype_index, num_argtypes, num_programs,
        ...
        def create_cell_scopes():
            ...
            self.target_var_key_and_embedding_arg_layer = [nn.Linear(2*self.argtype_embed_dim, self.argtype_embed_dim) \
            ...
            self.target_var_key_and_embedding_var_layer = [nn.Linear(2*self.var_embed_dim, self.var_embed_dim) \
                                                           for i in xrange(self.max_arguments)]
            ...
            self.update_attention_layer = [nn.Linear(self.max_num_var+self.npi_core_dim,self.max_num_var) \
            ...
            rel_embedding_mat = nn.Parameter(torch.tensor(rel_embedding, requires_grad=True, device=self.device, dtype=self.dtype_float))
            self.rel_embedding = nn.Embedding(rel_embedding.shape[0], rel_embedding.shape[1], _weight=rel_embedding_mat)
            type_embedding_mat = nn.Parameter(torch.tensor(type_embedding, requires_grad=True, device=self.device))
            ...
            word_embeddings_mat = nn.Parameter(torch.tensor(vocab_embed, requires_grad=True, device=self.device, dtype=self.dtype_float))
            ...
            program_embedding_mat = nn.Parameter(torch.tensor(np.random.normal(-max_val, max_val, [self.num_progs, self.prog_embed_dim]), \
            ...
            self.program_keys = nn.Parameter(torch.tensor(np.random.normal(-max_val, max_val, [self.num_progs, self.prog_key_dim]), \
                                             requires_grad=True, device=self.device, dtype=self.dtype_float))
            self.program_to_argtype_table = nn.Parameter(torch.tensor(program_to_argtype_table, requires_grad=True, device=self.device, \
                                                         dtype=self.dtype_int64))
            self.program_to_targettype_table = nn.Parameter(torch.tensor(program_to_targettype_table, requires_grad=True, \
                                                            device=self.device, dtype=self.dtype_int64))
            max_val = 6. /np.sqrt(self.num_argtypes + self.argtype_embed_dim)
            argtype_embedding_mat = nn.Parameter(torch.tensor(np.random.normal(-max_val, max_val, \
            ...
            self.critic_layer = nn.Linear(self.npi_core_dim+self.var_embed_dim, 1)
        create_cell_scopes()
...
#            beam_properties['check_penalization'] = [torch.zeros([self.batch_size,1], device=self.device) for beam_id in xrange(self.beam_size)]
            ...
            to_return_action_sequence = dict.fromkeys(['program_type','argument_type','target_type',\
                                                       'target_table_index','argument_table_index','states_for_advantage'])
            for key in ['program_type','argument_type','target_type','target_table_index','argument_table_index', 'states_for_advantage']:
                to_return_action_sequence[key] = [[] for beam_id in xrange(self.beam_size)]

            self.entropy = torch.tensor(0, dtype=self.dtype_float)
            feasible_progs_for_last_timestep = self.get_feasible_progs_for_last_timestep()

            for t in xrange(self.num_timesteps):
                entropy = torch.tensor(0, dtype=self.dtype_float)
                # =============================================================================
                ...
                for beam_id in xrange(self.beam_size):
                    ...
                    complete_action_probs = torch.clamp(complete_action_probs,self.eps,1)
...
#                top_scores, indices_top_scores = torch.topk(current_score, k = self.beam_size)
                top_scores, indices_top_scores = self.categorical_sample_k(current_score, k = self.beam_size)
                ...
                index_0 = torch.range(0,self.beam_size*self.batch_size-1).type(self.dtype_int64)
                ...
                b1 = (index_1==torch.zeros_like(index_1)).type(self.dtype_float)
                b2 = (index_1==self.num_progs-1*torch.ones_like(index_1)).type(self.dtype_float)
                ...
                for beam_id, true_prog_samples, true_var_samples in zip(xrange(self.beam_size),\
                    ...
                    condition = torch.max((target_types==torch.zeros_like(target_types)).type(self.dtype_float), (target_types==(self.num_progs-1*torch.ones_like(target_types))).type(self.dtype_float))
                    ...
                    adv_state = torch.cat([beam_properties['h'][beam_id], sentence_state], dim=1)
                    adv_state = adv_state.view([self.batch_size,-1])
                    to_return_action_sequence['states_for_advantage'][beam_id].append(adv_state)
                # _____________________________________________________________________________
                ###############################################################################

            # reshaping stuff so that it can be handled by main function
            to_return_action_sequence['next_state_value'] = []
            for beam_id in xrange(self.beam_size):
                state = torch.cat([beam_properties['h'][beam_id], sentence_state], dim=1)
                state = state.view([self.batch_size,-1])
                to_return_action_sequence['next_state_value'].append(state)
...
#        print num_variables_remaining[0].dtype,program_to_targettype_onehot.dtype
    ...
    def get_state_value(self, internal_state):
        state_value = self.critic_layer(internal_state)
        return state_value
...
#            input_1 = self.target_var_key_and_embedding_arg_layer[current_argtype_id](temp)
    ...
    def train(self, feed_dict2):
        with torch.no_grad():
            ...
            advantages = feed_dict2['advantage']
            self.per_step_logprob = torch.log(self.per_step_prob)
        def reinforce():
            # Calculating the ground truth "labels" as described above
            def prevent_nan(input_tensor):
                return torch.where(torch.isnan(input_tensor), torch.zeros_like(input_tensor, device=self.device), input_tensor)
            entropy_a2c = prevent_nan(self.per_step_prob * self.per_step_logprob).sum(2).mean()
            action_gain_a2c = prevent_nan(self.per_step_logprob * advantages).mean()
            value_loss_a2c = prevent_nan(advantages.pow(2)).mean()
            total_loss_a2c = value_loss_a2c - action_gain_a2c - self.params['Rate_Entropy']*entropy_a2c
            return total_loss_a2c
...
#        temp = torch.stack([torch.randperm(self.num_progs) for _ in xrange(self.batch_size)],dim=0)
    ...
    def categorical_sample_k(self,distribution,k):
        def sample_one(distribution):
            current_score = nn.functional.softmax(distribution.data)
            current_score_dist = torch.distributions.Categorical(current_score)
            return current_score_dist.sample()
        out2_ind = []
        temp_dist = distribution
        for _ in xrange(k):
            current_sample = sample_one(temp_dist)
            out2_ind.append(current_sample)
            temp = self.one_hot(current_sample, depth=temp_dist.shape[-1])
            temp = temp.type(temp_dist.dtype)
            temp = -1*sys.maxint*temp + (1-temp)
            temp_dist = temp_dist*temp
        out2_ind = torch.stack(out2_ind,dim=1)
        multiplicand_1 = self.one_hot(out2_ind, depth=distribution.shape[-1])
        multiplicand_1 = multiplicand_1.permute([1,0,2])
        out2_vals = torch.sum(torch.mul(multiplicand_1.type(distribution.dtype), distribution), dim=2)
        out2_vals = out2_vals.permute([1,0])
        return out2_vals, out2_ind
