...
class NPI(nn.Module):
    def __init__(self, params, none_argtype_index, num_argtypes, num_programs,
        ...
        def create_cell_scopes():
            ...
            if self.params['use_kb_emb_in_ques_rnn']:
                ...
                self.sentence_rnn = nn.GRU(input_size=self.text_embed_dim, \
                                       hidden_size=self.hidden_dim, batch_first=True)
...
#                flag_penalize = torch.min(to_penalize_score,dim=1,keepdim=True)[0]
            ...
            self.to_return_action_sequence = to_return_action_sequence
#            print 100*'%'
...
#            print 100*'%'
            return torch.exp(to_return_sequence_logprob), to_return_sequence_logprob, to_return_per_step_prob
...
#        print num_variables_remaining[0].dtype,program_to_targettype_onehot.dtype
    ...
    def sentence_encoder(self):
        ...
        rnn_inputs = self.word_embeddings(self.encoder_text_inputs_w2v)
        if self.params['use_kb_emb_in_ques_rnn']:
            rnn_inputs = torch.cat([rnn_inputs, self.encoder_text_inputs_kb_emb], dim = 2)
...
#            input_1 = self.target_var_key_and_embedding_arg_layer[current_argtype_id](temp)
    ...
    def train(self, feed_dict2):
        with torch.no_grad():
            ...
            self.instance_weights = torch.tensor(feed_dict2['instance_weight'], dtype=self.dtype_float, device=self.device).unsqueeze(dim=-1)
        def reinforce():
            ...
            loss_reinforce = torch.mul(loss_reinforce, self.instance_weights)
