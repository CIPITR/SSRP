...
#       variable_value_table = [] #list of len num_argtypes and each of the lists are of len max_num_var
            ...
            if program_type_id != self.program_type_vocab['none']:# and ('terminate' not in self.program_type_vocab or program_type_id!=self.program_type_vocab['terminate']):
                ...
                if target_value is None:
                    ...
                    if i<self.num_timesteps-1:
                        reward_till_timestep, _, _ = self.calculate_reward(target_value, target_type_id, cumulative_flag, num_lines_of_code, gold_entities, gold_ints, gold_bools, epoch_number, overall_step_number, False)
        ...
        if self.FLAG['All_none_programs'] or self.FLAG['First_program_none'] or self.FLAG['First_program_terminate']:
            ...
            reward, relaxed_reward, to_print = self.calculate_reward(target_value, target_type_id, flag, num_lines_of_code, gold_entities, gold_ints, gold_bools, epoch_number, overall_step_number)
        ...
        return reward, max_intermediate_reward, relaxed_reward, intermediate_mask, intermediate_reward_flag, to_print
    ...
    def calculate_reward(self, target_value, target_type_id, flag, num_lines_of_code, gold_entities, gold_ints, gold_bools, epoch_number, overall_step_number, print_flag = True):
        ...
        if self.argument_type_vocab_inv[target_type_id]=='set' or self.argument_type_vocab_inv[target_type_id]=='entity':
            ...
            if len(gold_entities_set)!=0 and gold_type=='set':
            ...
            if print_flag:
                to_print = 'In calculate reward: predicted_answer=set, gold_answer=',gold_type, ' reward =', reward, ' flag = ', flag, ' relaxed_reward =', relaxed_reward 
                if self.printing:
                    print to_print
                #else:
            ...
            if len(gold_ints)!=0 and gold_type=='int':
                if epoch_number <= self.relaxed_reward_till_epoch[0] and overall_step_number <= self.relaxed_reward_till_epoch[1]:
                    relaxed_reward += 1.
                ...
                if epoch_number > self.relaxed_reward_till_epoch[0] or overall_step_number > self.relaxed_reward_till_epoch[1]:
                    reward += -self.HIGH_NEGATIVE_REWARD
            if print_flag:
                to_print = 'In calculate reward: predicted_answer=int, gold_answer=',gold_type, 'reward=', reward, ' flag = ', flag, ' relaxed_reward =', relaxed_reward
                if self.printing:
                    print to_print
                #else:
                #    print 'reward =', reward, ' flag = ', flag, ' relaxed_reward =', relaxed_reward
            ...
            if len(gold_bools)!=0 and gold_type=='bool':
                if epoch_number <= self.relaxed_reward_till_epoch[0] and overall_step_number <= self.relaxed_reward_till_epoch[1]:
                    relaxed_reward += 1.
            ...
            if print_flag:
                to_print = 'In calculate reward: predicted_answer=bool, gold_answer=',gold_type, 'reward=', reward, ' flag = ', flag, ' relaxed_reward =', relaxed_reward
                if self.printing:
                    print to_print
                #else:
                #    print 'reward=', reward, ' flag = ', flag, ' relaxed_reward =', relaxed_reward
            ...
            if print_flag:
                to_print = 'In calculate reward: predicted_answer=', self.argument_type_vocab_inv[target_type_id], 'gold_answer=',gold_type,' reward =',reward, ' relaxed_reward =', -1
                if self.printing:
                    print to_print
                #else:
                #    print 'reward=', reward
        ...
        return reward, relaxed_reward, to_print
