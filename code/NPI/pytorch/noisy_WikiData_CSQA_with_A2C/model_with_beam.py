...
class NPI(nn.Module):
    def __init__(self, params, none_argtype_index, num_argtypes, num_programs,
        ...
        # @Future: needs to be putside init
#        self.optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate,beta1=0.9,beta2=0.999,epsilon=1e-08)
        ...
        def create_cell_scopes():
            ...
            # ####################################################################################################
            # self.npi_cell = tf.keras.layers.GRUCell(self.npi_core_dim)
            self.npi_scope = "npi_scope"
            self.npi_rnn = nn.GRU(input_size=self.npi_core_dim+self.var_embed_dim, hidden_size=self.npi_core_dim, batch_first=False)
            ...
            self.sentence_rnn = nn.GRU(input_size=self.wikidata_embed_dim+self.text_embed_dim, hidden_size=self.cell_dim, batch_first=True)
            self.sentence_rnn.flatten_parameters()
            # need to set the rnn stuff after discussing with amrita
            # ####################################################################################################
            ...
            self.target_var_key_and_embedding_arg_layer = [nn.Linear(2*self.argtype_embed_dim, self.argtype_embed_dim) for i in xrange(self.max_arguments)]
            self.get_target_var_key_and_embedding_var_scope = 'get_target_var_key_and_embedding_var_scope'
            self.target_var_key_and_embedding_var_layer = [nn.Linear(2*self.var_embed_dim, self.var_embed_dim) for i in xrange(self.max_arguments)]
            ...
            self.update_attention_layer = [nn.Linear(self.max_num_var+self.npi_core_dim,self.max_num_var) for i in xrange(self.num_argtypes)]

            self.batch_ids = torch.arange(0,self.batch_size,dtype = torch.int64)#, device=self.device)
            # tensor is of dimension batch_size x 1 i.e. [0, 1, 2, ... batch_size]

            rel_embedding_mat = torch.tensor(rel_embedding, requires_grad=True, device=self.device, dtype=torch.float32)
            self.rel_embedding = nn.Embedding(rel_embedding.shape[0], rel_embedding.shape[1], _weight=rel_embedding_mat)
            type_embedding_mat = torch.tensor(type_embedding, requires_grad=True, device=self.device)
            ...
            self.query_rel_atten = torch.tensor(np.random.normal(-max_val, max_val, [self.cell_dim, self.wikidata_embed_dim]), requires_grad=True, device=self.device)
            word_embeddings_mat = torch.tensor(vocab_embed, requires_grad=True, device=self.device, dtype=torch.float32)
            ...
            program_embedding_mat = torch.tensor(np.random.normal(-max_val, max_val, [self.num_progs, self.prog_embed_dim]), requires_grad=True, device=self.device, dtype=torch.float32)
            ...
            self.query_attention_h_mat = torch.tensor(np.random.normal(-max_val, max_val, [1, self.hidden_dim]), requires_grad=True, device=self.device, dtype=torch.float32)
            max_val = 6. / np.sqrt(self.wikidata_embed_dim + self.var_embed_dim)
            self.preprocessed_var_emb_mat = torch.tensor(np.random.normal(-max_val, max_val, [self.wikidata_embed_dim, self.var_embed_dim]), requires_grad=True, device=self.device, dtype=torch.float32)
            max_val = 6. / np.sqrt(self.num_progs + self.prog_key_dim)
            self.init_state = torch.zeros([1, self.cell_dim],requires_grad=True,device=self.device)
            self.program_keys = torch.tensor(np.random.normal(-max_val, max_val, [self.num_progs, self.prog_key_dim]), requires_grad=True, device=self.device, dtype=torch.float32)
            self.program_to_argtype_table = torch.tensor(program_to_argtype_table, requires_grad=True, device=self.device, dtype=torch.int64)
            self.program_to_targettype_table = torch.tensor(program_to_targettype_table, requires_grad=True, device=self.device, dtype=torch.int64)
            max_val = 6. /np.sqrt(self.num_argtypes + self.argtype_embed_dim)
            argtype_embedding_mat = torch.tensor(np.random.normal(-max_val, max_val, [self.num_argtypes, self.argtype_embed_dim]), requires_grad=True, device=self.device, dtype=torch.float32)
    ...
    def get_parameters(self):
        return self.program_keys, self.program_embedding, self.word_embeddings, self.argtype_embedding, self.query_attention_h_mat
    ...
    def manual_gather_nd(self, params,indices):
        ...
        f_params = params.contiguous().view([-1]+param_shape[number_of_axes:])
    ...
    def get_final_feasible_progs_for_last_timestep(self, feasible_progs, beam_properties, beam_id, feasible_progs_for_last_timestep, t):
        if t == self.num_timesteps-1:
            ...
            temp = torch.where((self.gold_target_type==beam_properties['target_type'][beam_id]), torch.ones_like(self.gold_target_type, device=self.device), torch.zeros_like(self.gold_target_type, device=self.device))
            ...
            temp2 = (1-self.last_step_feasible_program)*feasible_progs[beam_id] + self.last_step_feasible_program*torch.mul(feasible_progs[beam_id], feasible_progs_for_last_timestep)
    ...
    def inference(self, feed_dict):
        #with tf.device(tf.test.gpu_device_name()):
            ...
            for beam_id in xrange(self.beam_size):
                self.variable_embedding.append(torch.zeros([self.num_argtypes, self.batch_size, self.max_num_var, self.var_embed_dim], device=self.device))
                self.variable_keys.append(torch.zeros([self.num_argtypes, self.batch_size, self.max_num_var, self.var_key_dim], device=self.device))
                ...
                self.variable_mask.append(torch.zeros([self.num_argtypes, self.batch_size, self.max_num_var], device=self.device))
            self.encoder_text_inputs_w2v = torch.tensor(feed_dict['encoder_text_inputs_w2v'], device=self.device, dtype=torch.int64)
            self.preprocessed_var_mask_table = [[torch.tensor(feed_dict['preprocessed_var_mask_table'][i][j], device=self.device, dtype=torch.float32) for j in range(self.max_num_var)] for i in range(self.num_argtypes)]
            self.preprocessed_var_emb_table = [[torch.tensor(feed_dict['preprocessed_var_emb_table'][i][j], device=self.device, dtype=torch.float32) for j in range(self.max_num_var)] for i in range(self.num_argtypes)]
            self.encoder_text_inputs_kb_emb = torch.tensor(feed_dict['encoder_text_inputs_kb_emb'], device=self.device, dtype=torch.float32)
            self.kb_attention = torch.tensor(feed_dict['kb_attention'], device=self.device, dtype=torch.float32)
            self.progs_phase_1 = torch.tensor(feed_dict['progs_phase_1'], device=self.device, dtype=torch.int64)
            self.progs_phase_2 = torch.tensor(feed_dict['progs_phase_2'], device=self.device, dtype=torch.int64)
            self.gold_target_type = torch.tensor(feed_dict['gold_target_type'], device=self.device, dtype=torch.int64)
            self.randomness_threshold_beam_search = torch.tensor(feed_dict['randomness_threshold_beam_search'], device=self.device, dtype=torch.float32)
            self.DoPruning = torch.tensor(feed_dict['DoPruning'], device=self.device, dtype=torch.float32)
            self.last_step_feasible_program = torch.tensor(feed_dict['last_step_feasible_program'], device=self.device, dtype=torch.float32)
            self.bias_prog_sampling_with_last_variable = torch.tensor(feed_dict['bias_prog_sampling_with_last_variable'], device=self.device, dtype=torch.float32)
            self.bias_prog_sampling_with_target = torch.tensor(feed_dict['bias_prog_sampling_with_target'], device=self.device, dtype=torch.float32)
            self.required_argtypes = torch.tensor(feed_dict['required_argtypes'], device=self.device, dtype=torch.int64)
            self.relaxed_reward_multipler = torch.tensor(feed_dict['relaxed_reward_multipler'], device=self.device, dtype=torch.float32)
            sentence_state, attention_states = self.sentence_encoder()
            beam_properties = defaultdict(list)
            beam_properties['Model_Reward_Flag'] = [torch.zeros([self.batch_size], device=self.device) for beam_id in xrange(self.beam_size)]
            for beam_id in xrange(self.beam_size):
            ...
            unswitched_beam_properties['total_beam_score'] = [torch.zeros([self.batch_size], device=self.device)] + \
                                              [-30*torch.ones([self.batch_size], device=self.device) for beam_id in xrange(self.beam_size-1)]
            #beam_properties['total_beam_score'] = [tf.zeros([self.batch_size]) for beam_id in xrange(self.beam_size)]
            beam_properties['terminate'] = [torch.zeros([self.batch_size,1], device=self.device) for beam_id in xrange(self.beam_size)]
            beam_properties['length'] = [torch.zeros([self.batch_size,1], device=self.device) for beam_id in xrange(self.beam_size)]
            beam_properties['target_type'] = [torch.zeros([self.batch_size], device=self.device, dtype=torch.int64) for beam_id in xrange(self.beam_size)]
            ...
            for t in xrange(self.num_timesteps):
                ...
                for beam_id in xrange(self.beam_size):
                    ...
                    for _prog_sample_, _prog_embedding_, \
                        ...
                        complete_action_probs.append(torch.mul(input_var_sampled_probs, _program_prob_.contiguous().view([-1,1])))
                    ...
                    complete_action_probs =  complete_action_probs.contiguous().view([self.batch_size,-1])
                    ...
                    if self.params['none_decay'] is 1:
                        power_decay = 0.2
                        penalize_factor = torch.mul(beam_properties['none_count'][beam_id],\
                                                                  -1*torch.log(torch.tensor(math.pow(t+1,power_decay), device=self.device)*
                                                                   torch.ones_like(beam_properties['none_count'][beam_id], device=self.device)))
                        current_beam_score[beam_id] = current_beam_score[beam_id]+penalize_factor




                    beam_target_type = beam_properties['target_type'][beam_id].contiguous().view([self.batch_size, 1])
                    beam_gold_type = self.gold_target_type.contiguous().view([self.batch_size, 1])
                    ...
                    if t > 0:
                        penalize_none_start = torch.where(beam_target_type==0,\
                                                       torch.ones_like(beam_target_type, device=self.device),torch.zeros_like(beam_target_type, device=self.device)).type(torch.float32)
                ...
                current_score = current_score.contiguous().view([self.batch_size,-1])
                ...
                multiplier = self.one_hot(t*torch.ones([self.batch_size, self.beam_size], device=self.device), depth = self.num_timesteps, dtype=torch.float32)
                additand = torch.mul(multiplier, current_probs.contiguous().view([self.batch_size, self.beam_size,1]).repeat(1,1,self.num_timesteps))
                ...
                actual_var_samples_list = list(torch.unbind(torch.stack(actual_var_samples_list,dim = 2).type(torch.int64), dim = 0))
                ...
                index_0 = torch.range(0,self.beam_size*self.batch_size-1).type(torch.int64)
                # index_0 has shape [(beam_size*batch_size)]
                index_1 = true_program_sample.contiguous().view([-1])
                # index_1 has shape [(beam_size*batch_size)]
                index_2 = flattened_input_var_sample.contiguous().view([-1])
                # index_2 has shape [(beam_size*batch_size)]
                new_value = torch.ones_like(index_1, device=self.device)
                b1 = (index_1==torch.zeros_like(index_1)).type(torch.float32)
                b2 = (index_1==self.num_progs-1*torch.ones_like(index_1)).type(torch.float32)
                ...
                old_property_value = old_property_value.contiguous().view([-1, self.num_progs, int(math.pow(self.max_num_var,self.max_arguments))])
                ...
                old_property_value = old_property_value.contiguous().view([self.beam_size, self.batch_size, self.num_progs,\
                ...
                true_program_sample = true_program_sample.type(torch.int64)
                # _____________________________________________________________________________
                ###############################################################################
                ...
                for beam_id, true_prog_samples, true_var_samples in zip(xrange(self.beam_size),\
                    ...
                    condition = torch.max((target_types==torch.zeros_like(target_types)).type(torch.float32), (target_types==(self.num_progs-1*torch.ones_like(target_types))).type(torch.float32))
                    beam_properties['target_type'][beam_id] = torch.where(condition>0., beam_properties['target_type'][beam_id].type(torch.int64), target_types)
                    # _____________________________________________________________________________
    ...
    def get_feasible_progs(self, timestep, phase_elasticity):
        num_variables = [torch.transpose(torch.sum(self.variable_mask[i], dim=2, dtype=torch.int64), 1,0) for i in range(len(self.variable_mask))]
        ...
        reqd_programs = [torch.max(torch.mul(num_variables_remaining[i], program_to_targettype_onehot), dim=2)[0].type(torch.float32) for i in range(len(self.variable_mask))]
        ...
        num_variable_types = [torch.max(self.variable_mask[i], dim=2)[0].type(torch.int64) for i in range(len(self.variable_mask))]
        ...
        feasible_progs = [torch.prod(feasible_progs[i], dim=2).type(torch.float32) for i in range(len(self.variable_mask))]
        ...
        def separate_phases(arg):
            ...
            if timestep < self.max_num_phase_1_steps:
                ...
                multiplicand1 = self.progs_phase_1.type(torch.float32)
            else:
                temp = (1-phase_elasticity).repeat([1,self.num_progs])
                multiplicand1 = self.progs_phase_2.type(torch.float32)
        ...
        # =============================================================================
        # Hard Rules
        temp = self.one_hot(torch.zeros([self.batch_size], device=self.device), depth=self.num_progs, dtype=torch.float32)
    ...
    def add_preprocessed_output_to_variable_table(self, beam_id):
        ...
        for i in xrange(self.num_argtypes):
            ...
            for j in xrange(self.max_num_var):
                ones = i*torch.ones([1, self.batch_size], dtype=torch.int64, device=self.device)
                empties = self.empty_argtype_id*torch.ones([self.max_arguments-1, self.batch_size], dtype=torch.int64, device=self.device)
                ...
                target_types = i*torch.ones([self.batch_size], device=self.device, dtype=torch.int64)
    ...
    def sentence_encoder(self):
        ...
        attention_states = torch.transpose(sentence_outputs.contiguous().view([self.batch_size,self.max_len,-1]), 1,0)
    ...
    def state_encoding(self, e_state, target_var_embedding):
        merge = torch.cat([e_state.contiguous().view([self.batch_size, -1]), target_var_embedding], dim=1)
    ...
    def terminate_net(self, progs_taken, old_terminate):
        temp1 = torch.ones_like(progs_taken, device=self.device, dtype=torch.int64)
        temp2 = torch.zeros_like(progs_taken, device=self.device, dtype=torch.int64)
        # 0 is the None action
        terminate = torch.where((progs_taken==self.num_progs-1), temp1, temp2)
        terminate = terminate.contiguous().view([self.batch_size, 1]).type(old_terminate.dtype)
    ...
    def check_if_gold_target(self, beam_target_type, beam_gold_type, if_terminated):
        mask_same_type = torch.where((beam_target_type==beam_gold_type), torch.zeros_like(beam_target_type, device=self.device), \
                                  torch.ones_like(beam_target_type, device=self.device)).type(torch.float32)
        add_flag = torch.mul(mask_same_type,if_terminated)
    ...
    def prog_net(self, h, sentence_state, attention_states, query_attentions_till_now, feasible_progs, num_samples, terminate, last_target_type):
        ...
        last_target_type = last_target_type.contiguous().view([-1,1,1]).repeat(1,self.num_progs, self.max_arguments)
        programs_consuming_last_targettype = torch.max(torch.where((self.program_to_argtype_table==last_target_type),\
                        torch.ones_like(last_target_type, dtype=torch.int64, device=self.device), torch.zeros_like(last_target_type, device=self.device)), dim=2)[0].type(torch.float32)
        ...
        if self.concat_query_npistate:
            ...
            concat_hq = h
        concat_hq = concat_hq.contiguous().view([self.batch_size, -1])
        ...
        key = key.contiguous().view([-1, 1, self.prog_key_dim])
        ...
        if self.params['terminate_prog'] is True:
            temp = self.one_hot((self.num_progs-1)*torch.ones([self.batch_size], device=self.device), depth=self.num_progs, dtype=torch.float32)
    ...
    def input_var_net(self, h, arg_types, prog_sample, prog_embedding, kb_attention, beam_id, num_samples, terminate, past_program_variables):
        ...
        key = [key[i].contiguous().view([-1, 1, self.var_key_dim]) for i in xrange(self.max_arguments)]
        ...
        flattened_joint_var_dist = joint_var_dist.contiguous().view([self.batch_size,-1])
    ...
    def get_num_variables_till_now(self, beam_id, targettypes):
        t = torch.stack([targettypes, self.batch_ids], dim=1)
        var_mask = self.manual_gather_nd(self.variable_mask[beam_id], t)
        # var_mask is of dimension batch_size x max_num_var
        num_variables_till_now = torch.sum(var_mask, dim=1).type(torch.int64)
#        num_variables_till_now = num_variables_till_now.type(torch.int64)
        ...
        R_Flag = torch.zeros_like(num_variables_till_now, device=self.device, dtype=torch.float32)
    ...
    def update_attention(self, static_atten, h, i):
        ...
        inputs = torch.cat([static_atten,h.contiguous().view([self.batch_size,-1])], dim = 1)
    ...
    def mask_attention(self, static_atten, mask):
        ...
        masked_atten = masked_atten/new_l1_norm.contiguous().view([-1,1])
        return masked_atten

    def train(self, feed_dict2):
        self.Reward = torch.tensor(feed_dict2['reward'], dtype=torch.float32, device=self.device)
        ...
        self.IfPosIntermediateReward = torch.tensor(feed_dict2['IfPosIntermediateReward'], dtype=torch.float32, device=self.device)
        self.mask_IntermediateReward = torch.tensor(feed_dict2['mask_IntermediateReward'], dtype=torch.float32, device=self.device)
        self.IntermediateReward = torch.tensor(feed_dict2['IntermediateReward'], dtype=torch.float32, device=self.device)
        self.Relaxed_reward = torch.tensor(feed_dict2['Relaxed_reward'], dtype=torch.float32, device=self.device)

        def reinforce():
            #reward, ProgramProb, logProgramProb, per_step_prob, entropy, IfPosIntermediateReward, mask_IntermediateReward, IntermediateReward, OldBaseline
            #self.action_seq, self.ProgramProb, self.logProgramProb, BEAM_DEBUG_DATA, per_step_prob, entropy = self.inference()
            # here instead of running a forward _pass everytime we can sample from memory
            # ProgramProb is a tensor of shape batch_size x beam_size
            # grad_ProgramProb is a list of lengtth num_parameters
            # each element in list grad_ProgramProb is a tensor with
            # shape batch_size x beam_size x num_parameters
            # Reward is a tensor of shape batch_size x beam_size
    ...
    def recursive_joint_prob_generator(self,list_dists):
        if len(list_dists) == 2:
            dist_1 = list_dists[0].contiguous().view([-1,1])
            dist_2 = list_dists[1].contiguous().view([-1,1])
    ...
    def map_index_to_unflattened(self,number,shape):
        ...
        for divisor in shape[::-1]:
            remainder = torch.remainder(number,divisor).type(torch.int64)#number // divisor
            number = torch.div(number.type(torch.float32),float(divisor)).floor_()#number % divisor
            out.append(remainder)
            #print 'remainder ', remainder
        return out[::-1]

    def map_index_to_flattened(self,number, dimensions):
        number = number.type(torch.int64)
        one = torch.tensor(1,dtype=torch.int64,device=self.device)
        dimensions = list(torch.unbind(torch.tensor(dimensions, dtype=torch.int64, device=self.device), dim = 0))
...
#        inp1 = tf.to_float(inp1)
        inp1 = inp1.contiguous().view([dim, -1])
        ...
        z2 = inp3.contiguous().view([-1,dim])
        z3 = torch.mul(z2,z1)
        update_input = torch.sum(z3,dim = 1)

        m1 = torch.sum(z1, dim = 1)
        m1 = 1-m1
        inp1 = inp1.contiguous().view([-1, dim])
        new_inp1 = torch.mul(inp1,m1)
    ...
    def beam_switch(self, old_prop_val, new_beam_ids):
        ...
        old_prop_val = old_prop_val.contiguous().view([self.beam_size, self.batch_size, -1])
        new_prop_val = []
        ...
        new_prop_val = new_prop_val.contiguous().view(old_shape)
        return new_prop_val
...
#        return out_vals, tf.cast(out_ind, dtype=tf.int64)
    ...
    def one_hot(self,batch,depth,dtype=None):
        ...
        if torch.cuda.is_available():
            y_one_hot = torch.zeros(y_tensor.size()[0], n_dims,device=self.device).scatter_(1, y_tensor.cuda(), 1)
        else:
            y_one_hot = torch.zeros(y_tensor.size()[0], n_dims,device=self.device).scatter_(1, y_tensor, 1)
        y_one_hot = y_one_hot.contiguous().view(*(list(y.shape)+[-1]))
        if dtype is None:
            return y_one_hot.type(torch.int64)
