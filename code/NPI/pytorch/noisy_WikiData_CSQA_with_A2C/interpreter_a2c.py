...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  3 13:26:06 2018
...
class Interpreter():
    def __init__(self, wikidata_dir, num_timesteps, program_type_vocab, argument_type_vocab, printing, terminate_prog, relaxed_reward_strict, reward_function="jaccard", boolean_reward_multiplier = 0.1, relaxed_reward_till_epoch=[-1,-1], unused_var_penalize_after_epoch=[1000, 100000], length_based_penalization=False, discount_factor=0.8):
        np.random.seed(1)
...
#       variable_value_table = [] #list of len num_argtypes and each of the lists are of len max_num_var
        ...
        returns =np.zeros([self.num_timesteps])#np.array([None for _ in xrange(self.num_timesteps)]) #np.zeros([self.num_timesteps])
#        terminates = np.zeros([self.num_timesteps])
        terminates = 0
#        noops = np.zeros([self.num_timesteps])
...
#                returns[0:i+1] = -1*self.HIGHEST_NEGATIVE_REWARD
                returns[i] = -1*self.HIGHEST_NEGATIVE_REWARD
                returns[0:i] = self.discount_factor*returns[0:i] - self.HIGHEST_NEGATIVE_REWARD
                break
...
#                    returns[0:i+1] = -1*self.HIGHEST_NEGATIVE_REWARD
                    ...
                    temp = self.calculate_reward(target_value, target_type_id, cumulative_flag, num_lines_of_code, \
                                                 gold_entities, gold_ints, gold_bools, epoch_number, \
                                                 overall_step_number, False)
#                    returns[0:i+1] = temp[0]
                    returns[i] = temp[0]
                    returns[0:i] = self.discount_factor*returns[0:i] + temp[0]
                    if i<self.num_timesteps-1:
                        reward_till_timestep, _ = temp
        ...
        return reward, max_intermediate_reward, relaxed_reward, intermediate_mask, intermediate_reward_flag, returns, terminates
