from model_with_beam import NPI
...
#                self.optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate,beta1=0.9,beta2=0.999,epsilon=1e-08)
    ...
    def feeding_dict2(self, reward, ProgramProb, logProgramProb, per_step_prob, entropy, \
                        IfPosIntermediateReward, mask_IntermediateReward, IntermediateReward, relaxed_rewards):
        feed_dict = {}
        feed_dict['reward'] = reward.astype(np.float32)
        feed_dict['ProgramProb'] = ProgramProb.numpy().astype(np.float32)
        feed_dict['logProgramProb'] = logProgramProb.numpy().astype(np.float32)
        feed_dict['per_step_prob'] = per_step_prob.astype(np.float32)
        feed_dict['entropy'] = entropy.numpy().astype(np.float32)
    ...
    def forward_pass_interpreter(self, batch_orig_context, a_seq, per_step_probs, \
        ...
        program_probabilities = program_probabilities.numpy()
        ...
        def handle_variable_value_table(key,beam_id,timestep,batch_id):
            if key is not 'variable_value_table':
                new_a_seq[batch_id][beam_id][key][timestep] = a_seq[key][beam_id][timestep][batch_id].numpy()
            else:
    ...
    def perform_validation(self, batch_dict, epoch, overall_step_count):
        ...
        a_seq, program_probabilities, logprogram_probabilities, beam_props, per_step_probs, entropy = self.model.inference(feed_dict1)
        # reshaping per_step_probs for printability
        per_step_probs = per_step_probs.numpy()
    ...
    def perform_training(self, batch_dict, epoch, overall_step_count):
        ...
        if self.param['Debug'] == 0:
            ...
            if self.param['train_mode'] == 'ml':
                ...
                rescaling_term_grad = reward
            feed_dict2 = self.feeding_dict2(reward, program_probabilities, logprogram_probabilities, per_step_probs, entropy, intermediate_rewards_flag, mask_intermediate_rewards, intermediate_rewards, relaxed_rewards)
